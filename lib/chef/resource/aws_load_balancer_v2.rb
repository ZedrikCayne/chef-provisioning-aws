...
class Chef::Resource::AwsLoadBalancerV2 < Chef::Provisioning::AWSDriver::AWSResource
...
#   def aws_object
#     result = driver.elbv2.load_balancers[name]
...
# end
  ...
  def aws_object
    result=nil
    begin
      result = driver.elbv2.describe_load_balancers({ load_balancer_names: [name] }).load_balancer_descriptions
      if result.length == 1
        result = result[0]
      else
